        ...
        size_t size, int usage, buffer_handle_t* pHandle)
{
    ...
    //////add by cjf
    if(gRGB565_2_Luma) {
        bufferSize = m->finfo.line_length * m->info.yres * LUMA_2_565;
    }
    ...
    intptr_t paddr = intptr_t(m->framebuffer->pbase);

    if(gRGB565_2_Luma) {
        vaddr = intptr_t(m->buffer_base);
        paddr = intptr_t(m->buffer_base);
    }
    ...
    hnd->base = vaddr;
    hnd->pbase = paddr;
    //LOGI("----vaddr = 0x%x, paddr = 0x%x \n",vaddr, paddr);
    hnd->offset = vaddr - intptr_t(m->framebuffer->base);

    if(gRGB565_2_Luma) {
        hnd->offset = vaddr - intptr_t(m->buffer_base);
    }

    //LOGD("%d->%d: hnd->base=0x%08x hnd->offset=%d (framebuffer->base=0x%08x) \n",
      //  getpid(),gettid(),hnd->base, hnd->offset, m->framebuffer->base);
    *pHandle = hnd;
    return 0;
}
...
try_ashmem:
        ...
        if (err == 0) {
            ...
            if (offset < 0) {
				...
				err = ioctl(fd, PMEM_GET_PHYS, &sub);
				if(err == 0)
				pbase = (void*)sub.offset;
            ...
            format == HAL_PIXEL_FORMAT_YCbCr_422_SP) 
    {
        ...
        switch (format) {
            ...
            case HAL_PIXEL_FORMAT_RGBA_4444:
                bpp = 2;
                break;
			case HAL_PIXEL_FORMAT_L_8://hjk
				bpp = 1;
				break;
			case HAL_PIXEL_FORMAT_YA_44://hjk
				bpp = 1;
				break;
            default:
                return -EINVAL;
        ...
        buffer_handle_t handle)
{
    ...
    if (hnd->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) {
        ...
        if(gRGB565_2_Luma) {
            index = (hnd->base - (int)m->buffer_base) / (m->finfo.line_length * m->info.yres * LUMA_2_565);
        }
        ...
        hw_device_t** device)
{
    ...
    if (!strcmp(name, GRALLOC_HARDWARE_GPU0)) {
        ...
        if (property_get("ro.rgb565-2-luma", property, NULL) > 0) {
            if (atoi(property) == 1) {
                LOGW("jeffy ro.rgb565-2-luma gralloc");
                gRGB565_2_Luma = 1;
            }
        }
