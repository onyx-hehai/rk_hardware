...
{
    char driver_status[PROPERTY_VALUE_MAX];
    int count = 50; /* wait at most 20 seconds for completion */
...
int wifi_start_supplicant()
{
    ...
    /*
     * It's better to wait for control interface to be ready.
	 * -- Yongle Lai
     */
    count = 50; /* About 5 seconds */
    pi = NULL;

    LOGD("Waiting for wpa_supplicant control interface..\n");
    while (count-- > 0) {
#ifdef HAVE_LIBC_SYSTEM_PROPERTIES
        if (pi == NULL) {
            pi = __system_property_find(SUPP_IFACE_PROP_NAME);
        }
        if (pi != NULL) {
            __system_property_read(pi, NULL, supp_status);
            if (strcmp(supp_status, "ready") == 0) {
    		LOGD("Done, count=%d\n", count);
                return 0;
            }
        }
#else
        if (property_get(SUPP_IFACE_PROP_NAME, supp_status, NULL)) {
            if (strcmp(supp_status, "ready") == 0)
                return 0;
        }
#endif
...
int wifi_connect_to_supplicant()
{
    ...
    if (!property_get(SUPP_PROP_NAME, supp_status, NULL)
            || strcmp(supp_status, "running") != 0) {
        LOGE("Supplicant not running, cannot connect, SUPP_PROP_NAME=%s", supp_status);
        return -1;
    }
    LOGD("SUPP_PROP_NAME is %s", supp_status);

    /* Make sure supplicant is ready at all. */
    while (count-- > 0) {
        if (property_get(SUPP_IFACE_PROP_NAME, supp_status, NULL)) {
            if (strcmp(supp_status, "ready") == 0)
				break;
    ...
    if (count <= 0)
    {
    	LOGD("Waiting for WPA_SUPP to be ready timeout. status=%s", supp_status);
        return -1;
    }
    ...
    if (monitor_conn == NULL) {
        LOGE("Unable to open monitor to supplicant on \"%s\": %s",
             ifname, strerror(errno));
    ...
    if (wpa_ctrl_attach(monitor_conn) != 0) {
        LOGE("Unable to attach monitor to supplicant on \"%s\": %s",
             ifname, strerror(errno));
    ...
    LOGD("Connect to supplicant on %s successfully.", ifname);
