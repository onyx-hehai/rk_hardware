...
//////////add by cjf
static const char *idle_state = "idle";
static const char *standby_state = "standby";
static const char *wake_state = "wake";
...
initialize_fds(void)
{
    ...
    // XXX: not this:
    if (g_initialized == 0) {
        if(open_file_descriptors(PATHS) < 0) {
            LOGE("initialize_fds failed g_error=%d\n", g_error);
            return;
        }
        g_initialized = 1;
    }
